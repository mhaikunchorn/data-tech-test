...
class TestTransformer:
    ...
    # # TODO: Task 2
    # @mark.notimplemented
    def test__split_customers(self, order_data_instance):
        df = order_data_instance
        transformer = Transformer()

        split_customers_low, split_customers_high = transformer.split_customers(df, 2000)

        assert np.all(split_customers_low['amount'] == [10, 30, 40, 60, 70, 80])
        assert np.all(split_customers_high['amount'] == [2000, 5000])